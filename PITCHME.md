...
例題の「車の設計図」クラスを使って試してみます。

「self」を明示的に引数として記述することによって，
「私の車」が 「車の設計図」クラスのインスタンスである場合、
メソッドを呼び出すこれら2つの方法の理論的同等性が示すことができる。
...
#### Q.メソッドになぜ「self」が必要なの2

#### A. クラスを動的に変更する機能を提供するため

メソッドに「self」が必要な理由のもう一つの理由として
対応するメソッドを作成する関数をクラスに追加することにより、
クラスを動的に変更する機能です。
たとえば、次のように空クラスに関数を追加することにより
例題の「車の設計図」と完全に同等のクラスを作成できます。
...
#### A.PEPで「self」の使用が推奨されているからです。

「メソッドになぜ「self」が必要なの2」でも示したように、「self」を「自分自身」という文字列であっても正しく動作します。
