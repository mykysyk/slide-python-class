...
積極的に日本語化していますが実際に開発する時は日本語で書かないで下さい。
...
@[3,6,10](「メソッドのselfってなぜ必要？」と思ったかた)
@[3,6,10](ここでは話が高度なため時間が足りなくなるので話しませんが)
@[3,6,10](Pythonの生みの親Guidoがブログエントリでそのことを説明しています)
@[3,6,10](気になるかたはURLを参照してください)
@[3,6,10](http://neopythonic.blogspot.com/2008/10/why-explicit-self-has-to-stay.html)
@[3,6,10](話をもどします)
...
@[17](「私の車.色変えて('黒')」を使うと)
@[17,10-13](クラス側では「self.色」に「黒」がセットされ)
@[17,10-13](「メッセージ」と「self.色」が結合され)
...
@[1-9](関数にするとコードが短く見えますが)
@[11](関数ではデータを保持できないのでこちらに移動しています)
@[1,5](また、クラスでは「self」であった引数が)
@[1,5](関数では「車データ」になっています)
@[11-13](関数の場合はデータの管理を自分自身で行う必要があるので)
@[5,6,13](データに変更を加える場合は)
...
@[1,2](特殊メソッドの「\__\init\_\_」を作成し)
@[1,2](「注文リスト」を配列で初期化させる)
